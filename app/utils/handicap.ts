...
// The USGA WHS Formula (2020 Update)
export const calcUSGACourseHandicap = (index: number, slope: number, rating: number, par: number) => {
  const ch = (index * (slope / 113)) + (rating - par);
  return Math.round(ch);
};

// Your Custom League Calculation
export const calculateLeagueHandicap = async ($db: any, playerId: string, leagueDocId: string, currentGhin: number, coursesMap: Map<string, any>) => {
  try {
    ...
    // Querying the permanent archive for past scores
