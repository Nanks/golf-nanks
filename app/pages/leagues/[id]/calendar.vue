  ...
  <div class="max-w-2xl mx-auto select-none pb-32">
    <template v-if="leagueData">
        ...
        <template #action>
          <ClientOnly>
            <button 
              ...
              :class="isAdminMode ? 'bg-amber-500 text-white shadow-amber-500/20' : 'bg-white dark:bg-stone-800 text-stone-500 border border-stone-200 dark:border-stone-800'"
      ...
      <div class="flex items-center justify-center gap-8 mb-6">
        <button @click="prevYear" :disabled="selectedYear <= 2016" class="p-2 rounded-full bg-stone-200/50 dark:bg-stone-800 disabled:opacity-20 text-stone-400 active:scale-90 active:text-emerald-500">
          <Icon name="mdi:chevron-left" class="size-6" />
        </button>
        <div class="text-center min-w-[80px]">
          <span class="text-primary text-3xl tabular-nums">{{ selectedYear }}</span>
        </div>
        <button @click="nextYear" :disabled="selectedYear >= currentYear" class="p-2 rounded-full bg-stone-200/50 dark:bg-stone-800 disabled:opacity-20 text-stone-400 active:scale-90 active:text-emerald-500">
      ...
      <div class="space-y-3 px-1">
        <ClientOnly>
          <button v-if="isAdminMode" @click="openAddModal" class="w-full p-4 border-2 border-dashed border-stone-200 dark:border-stone-800 rounded-xl text-[10px] font-black uppercase tracking-widest text-stone-400 active:border-emerald-500 active:text-emerald-500 flex items-center justify-center gap-2 mb-4">
        ...
        <div v-for="event in events" :key="event.id" 
          ...
          :class="{ 'active:scale-[0.98] active:bg-stone-50 dark:active:bg-stone-900/50 cursor-pointer': isClickable(event) }"
        >
          ...
          <div class="w-px h-10 bg-stone-200 dark:bg-stone-800 shrink-0"></div>
          ...
          <div class="flex items-center gap-3 shrink-0">
            ...
            <ClientOnly>
              <div v-if="isAdminMode" class="flex gap-1">
                <button @click.stop="openEditModal(event)" class="p-1.5 text-stone-300 active:text-amber-500"><Icon name="mdi:pencil-circle" class="size-7" /></button>
                <button @click.stop="promptDelete(event)" class="p-1.5 text-stone-300 active:text-red-500"><Icon name="mdi:close-circle-outline" class="size-6" /></button>
              </div>
              <Icon v-else-if="isClickable(event)"
                :name="(event.iso === todayIso && isLeagueLive) ? 'mdi:poker-chip' : 'mdi:chevron-right'" 
                :class="(event.iso === todayIso && isLeagueLive) ? 'text-emerald-500 animate-pulse' : 'text-stone-300 dark:text-stone-600'"
    ...
    <div v-else class="flex flex-col items-center justify-center pt-32 space-y-4">
      ...
      <p class="text-secondary text-[10px] uppercase font-black tracking-widest">Syncing Calendar...</p>
    </div>
...
const leagueId = route.params.id // Using standardized route param

const leagueData = computed(() => dataStore.leagues.find(l => l.id === leagueId))

// Safe-guarded Admin check
...
const handleCardClick = (event) => {
  ...
  const path = (event.iso === todayIso && isLeagueLive.value) 
    ? `/leaderboard/${leagueId}/${event.iso}/live?from=calendar` 
    : `/leaderboard/${leagueId}/${event.iso}/history`
...
const getStatusUI = (status, iso) => {
  if (!status) return iso < todayIso ? { icon: 'mdi:check-circle-outline', color: 'text-stone-400' } : null
  ...
  if (['rain', 'mdi:weather-pouring', 'mdi:cancel'].includes(s)) return { icon: 'mdi:weather-pouring', color: 'text-stone-400' }
  if (s === 'handicap') return { icon: 'mdi:calculator', color: 'text-amber-500' }
  return s.startsWith('mdi:') ? { icon: s, color: 'text-stone-400' } : (iso < todayIso ? { icon: 'mdi:check-circle-outline', color: 'text-stone-400' } : null)
}
